...
class Bill_App:
    def __init__(self,root):
        ...
        self.db_connection = mysql.connector.connect(
            ...
            password="Hrhk@9090",  # Your database password
        ...
        def decrement(variable):
            if variable.get() > 0:  # Prevent negative values
                variable.set(variable.get() - 1)

        # Function to create a row of labels, entries, and buttons
        def create_item_row(row_index, label_text, variable):
            label = Label(F2, text=label_text, bd=2, font=(ft_style, 15, "bold"), bg=bg_color, fg="lightgreen")
            label.grid(row=row_index, column=0, padx=10, pady=10, sticky="w")

            Button(F2, text="-", command=lambda: decrement(variable), width=2).grid(row=row_index, column=1, padx=3, pady=8)
            
            entry = Entry(F2, width=4, textvariable=variable, font=(ft_style, 15, "bold"), bd=2, bg="white")
            entry.grid(row=row_index, column=2, padx=8, pady=8, sticky="e")
            
            Button(F2, text="+", command=lambda: increment(variable), width=2).grid(row=row_index, column=3, padx=3, pady=8)

        # Bath Soap
        create_item_row(0, "Bath Soap", self.bth_soap)

        # Shampoo
        create_item_row(1, "Shampoo", self.shmpo)

        # Conditioner
        create_item_row(2, "Conditioner", self.conditioner)

        # Facewash
        create_item_row(3, "Facewash", self.facewash)

        # Lotion
        create_item_row(4, "Lotion", self.lotion)

        # Hair Oil
        create_item_row(5, "Hair Oil", self.hairoil)

        # Sunscreen
        create_item_row(6, "Sunscreen", self.sunscreen)

        # Moisturizer
        create_item_row(7, "Moisturizer", self.moisture)

        # Foundation
        create_item_row(8, "Foundation", self.foundation)
        ...
        # Function to create a row of labels, entries, and buttons for fruits
        def create_fruit_row(row_index, label_text, variable):
            label = Label(F3, text=label_text, bd=2, font=(ft_style, 15, "bold"), bg=bg_color, fg="lightgreen")
            label.grid(row=row_index, column=0, padx=10, pady=10, sticky="w")

            Button(F3, text="-", command=lambda: decrement(variable), width=2).grid(row=row_index, column=1, padx=3, pady=8)
            
            entry = Entry(F3, width=4, textvariable=variable, font=(ft_style, 15, "bold"), bd=4, bg="white")
            entry.grid(row=row_index, column=2, padx=8, pady=8, sticky="e")
            
            Button(F3, text="+", command=lambda: increment(variable), width=2).grid(row=row_index, column=3, padx=3, pady=8)

        # Apples
        create_fruit_row(0, "Apples", self.apple)

        # Bananas
        create_fruit_row(1, "Banana", self.banana)

        # Grapes
        create_fruit_row(2, "Grapes", self.grapes)

        # Guava
        create_fruit_row(3, "Guava", self.guava)

        # Pears
        create_fruit_row(4, "Pears", self.pears)

        # Pomegranate
        create_fruit_row(5, "Pomegranate", self.pomergranates)

        # Apricot
        create_fruit_row(6, "Apricot", self.apricot)

        # Dragon Fruit
        create_fruit_row(7, "Dragon Fruit", self.dragon)

        # Pineapple
        create_fruit_row(8, "Pineapple", self.pineapple)
...
# =======================Vegtables=====================================
        ...
        # Function to create a row of labels, entries, and buttons for vegetables
        def create_vegetable_row(row_index, label_text, variable):
            label = Label(F4, text=label_text, bd=2, font=(ft_style, 15, "bold"), bg=bg_color, fg="lightgreen")
            label.grid(row=row_index, column=0, padx=10, pady=10, sticky="w")

            Button(F4, text="-", command=lambda: decrement(variable), width=2).grid(row=row_index, column=1, padx=3, pady=8)
            
            entry = Entry(F4, width=4, textvariable=variable, font=(ft_style, 15, "bold"), bd=4, bg="white")
            entry.grid(row=row_index, column=2, padx=8, pady=8, sticky="e")
            
            Button(F4, text="+", command=lambda: increment(variable), width=2).grid(row=row_index, column=3, padx=3, pady=8)

        # Potato
        create_vegetable_row(0, "Potato", self.potatao)

        # Tamato
        create_vegetable_row(1, "Tamato", self.tamato)

        # Mushroom
        create_vegetable_row(2, "Mushroom", self.mushroom)

        # Carrot
        create_vegetable_row(3, "Carrot", self.carrot)

        # Peas
        create_vegetable_row(4, "Peas", self.peas)

        # Bitter Gourd
        create_vegetable_row(5, "Bitter Gourd", self.bittergouard)

        # Pumpkin
        create_vegetable_row(6, "Pumpkin", self.pumpkin)

        # Capsicum
        create_vegetable_row(7, "Capsicum", self.capsicum)

        # Cabbage
        create_vegetable_row(8, "Cabbage", self.cabbage)
        ...
        # Function to create a row of labels, entries, and buttons for beverages
        def create_beverage_row(row_index, label_text, variable):
            label = Label(F5, text=label_text, bd=2, font=(ft_style, 15, "bold"), bg=bg_color, fg="lightgreen")
            label.grid(row=row_index, column=0, padx=10, pady=10, sticky="w")

            Button(F5, text="-", command=lambda: decrement(variable), width=2).grid(row=row_index, column=1, padx=3, pady=8)
            
            entry = Entry(F5, width=4, textvariable=variable, font=(ft_style, 15, "bold"), bd=4, bg="white")
            entry.grid(row=row_index, column=2, padx=8, pady=8, sticky="e")
            
            Button(F5, text="+", command=lambda: increment(variable), width=2).grid(row=row_index, column=3, padx=3, pady=8)

        # Mountain Dew
        create_beverage_row(0, "Mountain Dew", self.dew)

        # Sprite
        create_beverage_row(1, "Sprite", self.sprite)

        # Limca
        create_beverage_row(2, "Limca", self.limca)

        # Frooti
        create_beverage_row(3, "Frooti", self.frooti)

        # Mixed Juice
        create_beverage_row(4, "Mixed Juice", self.mixed_juice)

        # Natural Juice
        create_beverage_row(5, "Natural Juice", self.natural_juice)

        # Coffee
        create_beverage_row(6, "Coffee", self.coffee)

        # Energy Drink
        create_beverage_row(7, "Energy Drink", self.energy_drink)

        # Fruit Beer
        create_beverage_row(8, "Fruit Beer", self.beer)
    ...
    def validate_customer_details(self):
        if not self.c_name.get().strip() or not self.c_name.get().isalpha():
            messagebox.showerror("Error", "Please enter a valid customer name (only letters).")
            return False
        if not self.c_phone.get().strip().isdigit() or len(self.c_phone.get()) != 10:
            messagebox.showerror("Error", "Please enter a valid phone number (10 digits).")
            return False
        if not self.is_valid_email(self.c_email.get().strip()):
            messagebox.showerror("Error", "Please enter a valid email address.")
            return False
        if not self.c_bill_no.get().strip().isdigit():
            messagebox.showerror("Error", "Please enter a valid bill number (only digits).")
            return False
        return True
    ...
    def bill_area(self):
        ...
        elif self.c_email.get()!=self.is_valid_email:
            messagebox.showerror("Error","Email is Invalid")
    ...
    #         customer_email = self.c_email.get()

    #         # Insert data into the database
    #         insert_query = "INSERT INTO bills (customer_name, customer_phone, customer_email, bill_number, total_bill, bill_data) VALUES (%s, %s, %s, %s, %s, %s)"
    #         self.db_cursor.execute(insert_query, (customer_name, customer_phone, customer_email, bill_number, total_bill, self.bill_data))
    #         self.db_connection.commit()  # Commit the transaction

    #         # Save bill to a text file
    #         with open(f"/home/dheeraj/Bills/{bill_number}.txt", "w") as f1:
    #             f1.write(self.bill_data)
    ...
    #             messagebox.showinfo("Bill Saved", "Bill saved successfully in the database!")
    ...
    def save_bill(self):
        ...
        if op > 0:
            ...
            pdf_directory = os.path.expanduser("/home/dheeraj/Bills/")
            pdf_filename = f"{customer_name}_{bill_number}.pdf"
            ...
            try:
                ...
                # Write the bill data to the PDF
                ...
                # Insert data into the database
                insert_query = "INSERT INTO bills (customer_name, customer_phone, customer_email, bill_number, total_bill, bill_data, pdf_path) VALUES (%s, %s, %s, %s, %s, %s, %s)"
                self.db_cursor.execute(insert_query, (customer_name, customer_phone, customer_email, bill_number, total_bill, self.bill_data, pdf_path))
                self.db_connection.commit()  # Commit the transaction

                messagebox.showinfo("Bill Saved", f"Bill saved successfully in the database and as PDF at {pdf_path}!")
            except mysql.connector.Error as err:
                messagebox.showerror("Database Error", f"Error: {err}")
    ...
    def save_pdf(self):
        ...
        if op > 0:
            ...
            try:
                ...
                # Insert PDF path into the database
                bill_number = self.c_bill_no.get()
                total_bill = self.Total_bill
                customer_name = self.c_name.get()
                customer_phone = self.c_phone.get()
                ...
                insert_query = "INSERT INTO bills (customer_name, customer_phone, customer_email, bill_number, total_bill, bill_data, pdf_path) VALUES (%s, %s, %s, %s, %s, %s, %s)"
                self.db_cursor.execute(insert_query, (customer_name, customer_phone, customer_email, bill_number, total_bill, self.bill_data, file_path))
                self.db_connection.commit()  # Commit the transaction

                messagebox.showinfo("Information", f"Bill saved as {file_path} and in the database!")
            except Exception as e:
                messagebox.showerror("Save Error", f"An error occurred: {e}")
        else:
            return
    ...
    #     if result:
    #         self.txtarea.delete('1.0', END)
    #         self.txtarea.insert(END, result[6])  # Assuming bill_data is the 7th column
    #         present = "yes"
    #     if present == "no":
    #         messagebox.showerror("Error", "Bill not found")
    
    # def find_bill(self):
    #     bill_number = self.search_bill.get()  # Get the bill number from the entry field
    #     customer_name = self.c_name.get()      # Get the customer name from the entry field

    #     try:
    #         # Query to find the bill in the database
    #         select_query = "SELECT * FROM bills WHERE bill_number = %s AND customer_name = %s"
    #         self.db_cursor.execute(select_query, (bill_number, customer_name))
    #         result = self.db_cursor.fetchone()  # Fetch one record
    #         # print(bill_number,customer_name)
    #         if result:
    #             # Assuming the result contains the following fields:
    #             self.c_bill_no.set(result[0])  # Bill number
    #             self.c_name.set(result[1])      # Customer name
    #             self.c_phone.set(result[2])     # Customer phone
    #             self.c_email.set(result[3])     # Customer email
    #             # You can set other fields as necessary based on your database schema

    #             # Optionally, display the bill data in the text area if applicable
    #             self.txtarea.delete('1.0', END)  # Clear previous text
    #             self.txtarea.insert(END, result[6])  # Assuming bill_data is in the 7th column

    #             messagebox.showinfo("Success", "Bill found successfully!")
    #         else:
    #             messagebox.showerror("Error", "Bill not found!")
    ...
    def find_bill(self):
        bill_number = self.c_bill_no.get().strip()  # Get the bill number from the entry field and trim spaces
        print(bill_number)

        try:
            # Query to find the bill in the database
            select_query = "SELECT * FROM bills WHERE bill_number = %s"
            self.db_cursor.execute(select_query, (bill_number,))
            result = self.db_cursor.fetchone()  # Fetch one record
            # print(result)
...
#             messagebox.showerror("Error",f"Failed to send email: {e}")
    def share(self):
        self.sender_email = "btcl220010159010gjust@gmail.com"
        self.receiver_email = self.c_email.get()
        self.app_password = "oldw cwxr fovq zudr"
        self.subj = f"Grocery Bill on your demand: {self.c_bill_no.get()}"
        self.body = f"""Dear {self.c_name.get()},\n 
    Thank you for your recent order! Your order number is {self.c_bill_no.get()}\n 
    Your Bill is attached with this email as your demand. Download from there : \n 
    ...
    Nitin Grocery Store\n Address: Near GJUST, Hisar"""
        ...
        bill_number = self.c_bill_no.get()
        customer_name = self.c_name.get()
        select_query = "SELECT pdf_path FROM bills WHERE bill_number = %s AND customer_name = %s"
        self.db_cursor.execute(select_query, (bill_number, customer_name))
        result = self.db_cursor.fetchone()
        ...
        if result:
            ...
            directory = os.path.expanduser("/home/dheeraj/Bills")  # User's home directory
            filename = f"{self.c_name.get()}_{self.c_bill_no.get()}"
            # file_path = os.path.join(directory, filename)
            k = sendpdf(self.sender_email, self.receiver_email, self.app_password, self.subj, self.body, address_of_file=directory,filename=filename)
            try:
                k.email_send()  # Assuming this method handles sending the email
    ...
    def show_qr_code_dialog(self, qr_image, timeout=30):
        ...
        qr_image = qr_image.resize((200, 200), Image.LANCZOS)  # Adjusted line
        qr_image_tk = ImageTk.PhotoImage(qr_image) 
